      ...
      callback: function (r) {
        if (r?.message?.user_appointment_availability) {
          frappe.set_route(
            ...
            r.message.user_appointment_availability,
          );
          return;
        }
        if (!r.message?.is_google_calendar_setup) {
          frappe.confirm(
            __(
              "You don't have a Google Calendar setup yet. Would you like to setup it first?",
            ),
            () => frm.trigger("setup_new_google_calendar"),
          ...
          frappe.confirm(
            __(
              "No Google Calendar is enabled for this user. Would you like to enable it first?",
            ),
            () => {
              if (r.message.google_calendar_id) {
                frappe.set_route(
                  "form",
                  "Google Calendar",
                  r.message.google_calendar_id,
                );
              } else {
                frappe.set_route("list", "Google Calendar", {
                  user: frm.doc.name,
                });
              }
            },
          ...
          frappe.confirm(
            __(
              "Google Calendar is not authorized for this user. Would you like to authorize it first?",
            ),
            () => {
              if (r.message.google_calendar_id) {
                frappe.set_route(
