...
def get_time_slots_for_day(appointment_group_id: str, date: str, user_timezone_offset: str) -> object:
    ...
    try:
        ...
        return send_http_response(404, {"message": "Appointment Group not found with the given ID."})

    user_start_time, user_end_time = get_date_start_end_time_for_given_timezone(date, user_timezone_offset)

    utc_start_time, utc_end_time = convert_datetime_to_utc(user_start_time), convert_datetime_to_utc(user_end_time)

    user_system_slots = get_user_appointment_availability_slots(appointment_group, utc_start_time, utc_end_time)
