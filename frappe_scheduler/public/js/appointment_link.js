  ...
  if (!doctype || doctype != frm.doctype) {
    ...
    frappe.ui.form.on(doctype, {
      refresh: function (frm) {
        ...
        frappe.call({
          method:
            "frappe_scheduler.frappe_scheduler.doctype.appointment_group.appointment_group.get_appointment_groups_from_doctype",
          args: {
            doctype: doctype,
          },
          callback: function (r) {
      ...
      view_appointment: function (frm) {
        frappe.call({
          method: "frappe_scheduler.overrides.event_override.get_events_from_doc",
          args: {
            doctype: doctype,
            docname: frm.docname,
            past_events: true,
      ...
      schedule_appointment: function (frm) {
        ...
        dialog.fields_dict.appointment_group.$input.on("change awesomplete-selectcomplete", function () {
          if (dialog.fields_dict.appointment_group.$input.val()) {
            frappe.call({
              method:
                "frappe_scheduler.frappe_scheduler.doctype.appointment_group.appointment_group.get_appointment_group_from_id",
              args: {
