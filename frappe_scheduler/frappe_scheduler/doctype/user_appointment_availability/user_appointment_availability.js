...
frappe.ui.form.on("User Appointment Availability", {
  refresh(frm) {
    ...
    if (frm.doc.__islocal) {
      ...
      frappe.call({
        method:
          "frappe_scheduler.frappe_scheduler.doctype.scheduler_settings.scheduler_settings.get_default_email_template",
        callback: function (r) {
  ...
  slug(frm) {
    ...
    if (frm.doc.slug === "" || frm.doc.slug === og_slug) {
      ...
      frappe.call({
        method:
          "frappe_scheduler.frappe_scheduler.doctype.user_appointment_availability.user_appointment_availability.is_slug_available",
        args: {
          slug: frm.doc.slug,
        },
        callback: function (r) {
