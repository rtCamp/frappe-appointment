...
def check_google_calendar_setup(user):
    ...
    g_calendars = frappe.get_all(
        "Google Calendar",
        filters={"user": user},
        fields=["name", "enable", "custom_is_google_calendar_authorized"],
    )

    if len(g_calendars) > 0:
        is_google_calendar_setup = True

    google_calendar_id = None
    if len(g_calendars) == 1:
        google_calendar_id = g_calendars[0].name

    is_google_calendar_enabled = True
    if not any([g_calendar.enable for g_calendar in g_calendars]):
        is_google_calendar_enabled = False
    is_google_calendar_authorized = True
    if not any([g_calendar.custom_is_google_calendar_authorized for g_calendar in g_calendars]):
